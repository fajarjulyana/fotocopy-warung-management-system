...
def migrate_database():
    ...
    try:
        ...
        # Check if invoices table exists and add warranty columns
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='invoices'")
        invoices_exists = cursor.fetchone()
        
        if invoices_exists:
            # Get current columns for invoices table
            cursor.execute("PRAGMA table_info(invoices)")
            invoice_columns = [column[1] for column in cursor.fetchall()]
            
            # Add warranty columns if they don't exist
            warranty_columns = {
            ...
            for column, definition in warranty_columns.items():
                if column not in invoice_columns:
                    try:
                        cursor.execute(f"ALTER TABLE invoices ADD COLUMN {column} {definition}")
                        print(f"Added warranty column to invoices: {column}")
                    except sqlite3.OperationalError as e:
                        print(f"Error adding invoice column {column}: {e}")
