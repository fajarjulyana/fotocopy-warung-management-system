...
def deposit():
    ...
    if request.method == 'POST':
        try:
            ...
            if amount < 1000:
                flash('Jumlah setoran minimal Rp 1.000!', 'error')
                return render_template('deposit.html', existing_savers=existing_savers)
...
@app.route('/saver/<int:saver_id>')
def saver_detail(saver_id):
    """Show saver details and transaction history"""
    saver = Saver.query.get_or_404(saver_id)
    transactions = Transaction.query.filter_by(saver_id=saver_id).order_by(Transaction.created_at.desc()).all()
    
    return render_template('saver_detail.html', saver=saver, transactions=transactions,
                         format_currency=format_currency,
                         format_date_indonesian=format_date_indonesian)


@app.route('/print_statement/<int:saver_id>')
def print_statement(saver_id):
    """Print saver statement"""
    saver = Saver.query.get_or_404(saver_id)
    transactions = Transaction.query.filter_by(saver_id=saver_id).order_by(Transaction.created_at.desc()).all()
    current_datetime = datetime.now()
    
    return render_template('print_statement.html', saver=saver, transactions=transactions,
                         format_currency=format_currency,
                         format_date_indonesian=format_date_indonesian,
...
def edit_entry(entry_id):
    """Legacy edit - not supported in new system"""
    flash('Edit tidak didukung dalam sistem baru. Silakan buat transaksi baru.', 'info')
    return redirect(url_for('index'))


@app.route('/delete/<int:entry_id>')
def delete_entry(entry_id):
    """Legacy delete - not supported in new system"""
    flash('Hapus tidak didukung dalam sistem baru untuk menjaga integritas data.', 'info')
    return redirect(url_for('index'))
...
def get_saver_balance(saver_name):
    ...
    if saver:
        ...
        return jsonify({
            'exists': False,
            'balance': 0,
            'formatted_balance': 'Penabung baru'
        })
...
if __name__ == '__main__':
  ...
  # app.run(host='0.0.0.0', port=5003, debug=True, ssl_context=('ssl_cert.pem', 'ssl_key.pem'))
